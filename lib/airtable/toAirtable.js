...
function insertNewRuns(ids, runs) {
    var inserts = runs.map(function (run) {
        var id = run.id, name = run.name, distance = run.distance, total_elevation_gain = run.total_elevation_gain, start_date = run.start_date, average_speed = run.average_speed, workout_type = run.workout_type, moving_time = run.moving_time, has_heartrate = run.has_heartrate, suffer_score = run.suffer_score, type = run.type;
        if (ids.includes(id)) {
